...
We prepare our matrix for WGCNA, where we need it to be such that rows -> samples and columns -> genes, so we transpose it. We also load our trait data.
```{r chunk dataWGCNA}
...
red <- blockwiseModules(input_mat,             
                          ...
                          deepSplit = deepSplit, #  0 to 4, to make it more sensitive and get more modules
                          pamRespectsDendro = F,
...
```{r module_trait, fig.width = 15, fig.height = 10, fig.align='center', eval=TRUE}
...
```
#### TESTING
This is a test to check if the heatmaps are informative enough.
...
for (c in 1:length(colorsA1)){
      module_colors == colorsA1[c]
      heatmap.2(t(input_mat[module_colors==colorsA1[c]]), scale = "row", col=col, density.info ="none", trace="none", cexCol=0.5, cexRow=0.8, margin=c(19,11), main = colorsA1[c], Colv = FALSE)
      }; dev.off()
```
