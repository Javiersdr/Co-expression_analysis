...
| Type of filtering | `r filter_type`|
| Expression threshold | `r num_thresh`|
| Number of samples for `num_thresh`| `r num_samples`|
| Quantile to keep genes based on mean or variance | `r quantile_thresh`|
...
if (filter_type == 1) {
  # Raw normalized expression filter
  data_matrix <- data_matrix[rowSums(data_matrix >= num_thresh, na.rm = TRUE) >= num_samples, ]
} else if (filter_type == 2) {
  # mean filter
  mean_expression <- rowMeans(data_matrix, na.rm = TRUE)
  mean_quantile <- quantile(mean_expression, quantile_thresh, na.rm = TRUE)
  data_matrix <- data_matrix[mean_expression > mean_quantile,]
} else if (filter_type == 3) {
  # variance filter
  var_expression <- rowVars(data_matrix, na.rm = TRUE)
  var_quantile <- quantile(var_expression, quantile_thresh, na.rm = TRUE)
  data_matrix <- data_matrix[var_expression > var_quantile,]  
}
```
